...
abstract class CMessageSource extends CApplicationComponent
{
	...
	public function translate_pavel($category,$language=null)
	{
		if($language===null)
			$language=Yii::app()->getLanguage();
		if($this->forceTranslation || $language!==$this->getLanguage())
			return $this->translateMessage_pavel($category,$language);
		else
			return $message;
	}
	...
	protected function translateMessage_pavel($category,$language)
	{
		$key=$language.'.'.$category;
		//echo 'key '.$key;
		//foreach ($this->_messages[$key] as $r){
		//	echo $r;
		...
		if(!isset($this->_messages[$key])) {
			 //$this->_messages[$key]=$this->loadMessages($category,$language);
			 $this->_messages[$key]=$this->loadMessages($category,$language);
			 //echo 'here '.$this->_messages[$key];
			// return $this->_messages[$key];
		}
		else{
			//echo $this->_messages[$key];
		}
		return $this->_messages[$key];
		/*
		if(isset($this->_messages[$key][$message]) && $this->_messages[$key][$message]!=='')
