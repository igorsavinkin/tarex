...
class YiiBase
{
	...
	public static function t_all_pavel($category,$params=array(),$source=null,$language=null)
	{
		if(self::$_app!==null)
		{
			if($source===null)
				$source=($category==='yii'||$category==='zii')?'coreMessages':'messages';
			if(($source=self::$_app->getComponent($source))!==null)
				$message=$source->translate_pavel($category,$language);
		}
		if($params===array())
			return $message;
		if(!is_array($params))
	...
	public static function t_all($category,$message,$params=array(),$source=null,$language=null)
	{
		$language='ru';
		if(self::$_app!==null)
		{
			if($source===null)
				$source=($category==='yii'||$category==='zii')?'coreMessages':'messages';
			if(($source=self::$_app->getComponent($source /*. '/' . $language . '/' . $category . '.php'*/ ))!==null) {
