...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<!--- КОНТРАГЕНТ --->
	<td  >
		<span style="white-space:nowrap;"><label><?php echo $model->StatusId != Events::STATUS_COMPLETED ? Yii::t('general','User in system, whom to lay off') : Yii::t('general','User in system, who has been laid off');  ?></label></span>
		<?php 			
			if ($UserRole < 2)  
			{
				$this->widget('ext.select2.ESelect2', array(
						...
						'data' => CHtml::listData(User::model()->findAll( array(
							'order'=>'name ASC', 
							'condition'=>"isEmployee = 1")), 'id','username'
							),
						'options'=> array('allowClear'=>true, 
							'width' => '200', 
							'placeholder' => '', 
			...
			{				
				$AllowedContractors = CHtml::listData(User::model()->findAll(array(
					'order'=>'name ASC',
					'condition'=>"organization=:organization AND isEmployee = 1 AND role > " . Yii::app()->user->role,
