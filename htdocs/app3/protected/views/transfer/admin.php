...
$this->pageTitle =Yii::t('general','Transfer');
...
"); 
?>
<h1><?php echo Yii::t('general','Transfer'); ?></h1><br> 
<?php 
if(Yii::app()->user->role<6){
	echo CHtml::link(Yii::t('general','Advanced Search'),'#',array('class'=>'search-button btn-win')); 
}
echo CHtml::link(Yii::t('general','Create') . ' ' . Yii::t('general', 'Transfer' ) , array('create', 'new'=>1), array( 'class' => 'btn-win')); ?>
...
// конец выбора действия которое надо применить при клике на запись в сетке
 ...
 $this->widget('zii.widgets.grid.CGridView', array(
	...
	'columns'=>array(
		...
		'Warehouse1' => array(          
			'header'=>Yii::t('general','Warehouse') . ' 1', 
			'value'=> 'Warehouse::model()->findByPk($data->Subconto1)->name', 
		),
		'Warehouse2' => array(          
			'header'=>Yii::t('general','Warehouse') . ' 2', 
			'value'=> 'Warehouse::model()->findByPk($data->Subconto2)->name', 
		), 
		'totalSum', 
		'Notes',
		array(
