...
?> 
	...
	<td >
		...
		<?php if ($UserRole<=2) echo $form->textField($model,'id',array('size'=>10,'maxlength'=>20)); 
			else echo $model->id; ?>
	...
	<td  >
		...
		<?php $model->totalSum = EventContent::getTotalSumByEvent($model->id); 
			echo '<b>', $model->totalSum, '</b>'; ?>
		<?php echo $form->error($model,'totalSum'); ?>
	</td> 
	
</tr><tr> 

	<td> <label><?php echo Yii::t('general', 'Customer (Contractor)'); ?></label>
		<?php 
		// ===== КОНТРАГЕНТ =====
		...
		<?php 	 
			if ($UserRole<2){
				$this->widget('ext.select2.ESelect2', array(
					...
					'data' => CHtml::listData(User::model()->findAll(array('condition'=>'isCustomer=1', 'order'=>'name ASC')), 'id','username'),
					'options'=> array('allowClear'=>true, 
						'width' => '200', 
						'placeholder' => '', 
						),
				)); 
				echo '<br>', CHtml::Link(Yii::t('general','Manage Customers') , array('user/admin', 'User[isCustomer]'=>'1'), array('target'=>'_blank')); 	
			}
	...
	<td colspan=3  valign=middle>
		<?php 
			if ($UserRole<=5){
				...
				echo $form->textArea($model,'Notes',array('rows'=>4, 'cols'=>60)); 
		...
		<span style='padding: 0px 5px 20px;margin-top:-25px;'>
			<?php echo CHtml::submitButton( Yii::t('general','OK'), array('class'=>'red', 'name'=>'OK')); ?>
		</span>
...
<div style='padding: 10px 0px 0px'><h2>
<?php  
