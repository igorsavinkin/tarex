...
$managerAndCanEdit = ($UserRole>3 && $UserRole<=5 && $CurrentStatusOrder < 4);
?>     
...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<td>
		...
		<?php 	//===== СМЕНА СТАТУСА у суммы ТОЛЬКО ВПЕРЁД ======
			if ($UserRole<=2 OR $managerAndCanEdit)
			{					
	...
	<? echo $form->labelEx($model,'parent_id');   
		if ($UserRole <= 2 OR ($model->isNewRecord && $UserRole>3 && $UserRole <= 5) )
		{   
			$this->widget('ext.select2.ESelect2', array(
					'model'=> $model,
					'attribute'=> 'parent_id', 
	...
	<td style='padding-left:10px;'>
		<?php 
			...
			if (($UserRole<=2) OR ($UserRole>3 && $UserRole<=5 && $CurrentStatusOrder<4)){
