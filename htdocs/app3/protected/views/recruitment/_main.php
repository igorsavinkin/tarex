...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<!--- Зарплата --->	
	<td>
		<label><?php echo Yii::t('general','Salary'); ?></label>
		<?php 	//===== СМЕНА у зарплаты ТОЛЬКО ВПЕРЁД ======
			if ($UserRole<=2 OR ($UserRole==3 && $CurrentStatusOrder<4))
			{					
				echo $form->textField($model,'totalSum');
			} else { 
				// если порядок статуса 4 и более  
			   echo $model->totalSum; 
			} 
		?> 	
	</td>
		...
		<?php 	
			...
			{	
				...
				foreach ($EventStatus as $r)					
					$AllowedForAccountantEventStatuses[$r->id] = yii::t('general',$r->name);
	
				if (!empty($AllowedForAccountantEventStatuses))
				{
					$this->widget('ext.select2.ESelect2', array(
					'model'=> $model,
					'attribute'=> 'StatusId',
					'data' => $AllowedForAccountantEventStatuses,
					'options'=> array('allowClear'=>true, 
						'width' => '150', 
						'placeholder' => '',						
						),
	...
	<!--- КОНТРАГЕНТ --->
	<td  >
		<span style="white-space:nowrap;"><label><?php echo $model->StatusId != Events::STATUS_COMPLETED ? Yii::t('general','User in system, whom to hire') : Yii::t('general','User in system, who has been hired');  ?></label></span>
		<?php 			
			if ($UserRole < 2)  
			{
				$this->widget('ext.select2.ESelect2', array(
						...
						'data' => CHtml::listData(User::model()->findAll( array(
							'order'=>'name ASC', 
							'condition'=>"isEmployee = 0 AND isSeller = 0")), 'id','username'
							),
						'options'=> array('allowClear'=>true, 
							'width' => '200', 
							'placeholder' => '', 
			...
			//Бухгалтерам и директорам разрешаем выбрать контрагента если статус "новый" или в работе"
			elseif ($CurrentStatusOrder<4 && $UserRole<=3)
			{				
				$AllowedContractors = CHtml::listData(User::model()->findAll(array(
					'order'=>'name ASC',
					'condition'=>"organization=:organization AND isEmployee = 0 AND isSeller = 0",
