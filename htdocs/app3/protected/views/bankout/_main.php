...
<?php $form=$this->beginWidget('CActiveForm', array(
		...
		<?php 	
			...
			{	
				...
				foreach ($EventStatus as $r)					
					$AllowedForBookkeeperEventStatuses[$r->id] = yii::t('general',$r->name);
				...
				if (!empty($AllowedForManagerEventStatuses))
				{
					...
					'model'=> $model,
					'attribute'=> 'StatusId',
					'data' => $AllowedForBookkeeperEventStatuses,
					'options'=> array('allowClear'=>true, 
						'width' => '150', 
						'placeholder' => '',						
						),
	...
	<? echo $form->labelEx($model,'parent_id');   
		if ($UserRole <= 2 OR ($model->isNewRecord && $UserRole==3 /* для бухгалтеров */) )
		{   
			$this->widget('ext.select2.ESelect2', array(
					'model'=> $model,
					'attribute'=> 'parent_id', 
					'data' => Events::ordersOfOrganization(Yii::app()->user->checkAccess(User::ROLE_ADMIN) ? 0 : Yii::app()->user->organization),
					'options'=> array('allowClear'=>true, 
						'width' => '200', 
						'placeholder' => '', 
						),
		...
		{
			$ParentOrder = Order::model()->findbypk($model->parent_id);
			if (!empty($ParentOrder))  
				echo CHtml::Link($ParentOrder->orderNotation(), array('order/update', 'id'=>$model->parent_id), array('target'=>'_blank'));
			else 
				echo  Yii::t('general','Order is not defined');		 
		}			
		?>  
	</td>
	<!--- Заказ Родитель --->
 ...
 <!--- Расчётный счёт  --->
	<td>
	<label><?php echo Yii::t('general','Current Account'); ?></label> 
	<?php 
		...
		{	
			...
			$this->widget('ext.select2.ESelect2', array(
				'model'=> $model,
				'attribute'=> 'Subconto1',
				'data' => CHtml::listData(Currentaccount::model()->findAll($condition), 'id','name'),
				'options'=> array('allowClear'=>true, 
					'width' => '200', 
					'placeholder' => ''),
			));				 
			echo '<br>', CHtml::Link(Yii::t('general','Edit Current Accounts') , array('currentaccount/admin'), array('target'=>'_blank')); 
		} else 
			echo Currentaccount::model()->findByPk($model->Subconto1)->name;			
		?>
	</td> 
  <!--- Расчётный счёт  --->
...
<div style='padding: 10px 0px 0px'>
	<?php  
	echo CHtml::link(Yii::t('general','Print payment order'),  array('printPPL', "id"=> $model->id), array('class' => 'btn-win')); 

	echo CHtml::link(Yii::t('general','Print payment order'). ' 2',  array('printPPL2', "id"=> $model->id), array('class' => 'btn-win')); 

	echo CHtml::link(Yii::t('general','Print payment order'). ' OrderController',  array('order/printPPL', "id"=> $model->id), array('class' => 'btn-win')); 
	?>
</div>
