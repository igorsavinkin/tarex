				...
				Ext.each(TemporaryStore.data.items, function(record){
					...
					if(TabsStore2!='Oversale Items'){
						...
						for (var i = 0; i < AlredyOrdered.length; i++) {
							...
							var SS= AlredyOrdered[i].indexOf(record.get('Currency'));
							if(SS!=-1){
								...
								if(SS!=-1){
									...
									CurrentCurrencyAmount+=record.get('Sum'); 
						...
						if (AddedAmount==0){
								AlredyOrdered.push(record.get('Currency')+'='+record.get('Sum')); 
					...
					for (var i = 0; i < AlredyOrderedWOversale.length; i++) {
						...
						//console.log(AlredyOrdered[i]);
						var SS= AlredyOrderedWOversale[i].indexOf(record.get('Currency'));
						if(SS!=-1){
							//console.log(element1[AlredyOrdered[i]]);
					...
					if (AddedAmount==0){
						AlredyOrderedWOversale.push(record.get('Currency')+'='+record.get('Sum')); 
