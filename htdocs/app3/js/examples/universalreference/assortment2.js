...
//appMain.AssortmentStore = Ext.extend(Ext.data.Store, {
Ext.define('appMain.AssortmentStore', {
     extend: 'Ext.data.Store'
    ,border:false
	,alias : 'widget.AssortmentStore'
    ,initComponent:function() {
        var config = { 
          	xtype: "store",
			id: 'assortmentStore',
			storeId: 'assortmentStore',
			remoteSort:true,
			autoLoad:true,
			pageSize: 20,
			fields:[		
			...
			proxy:{ 
				url: '../dbaccess/getTablePagingFilter.php', 
				params: { Table: 'assortmentFake' },
				method: 'get',
				reader: {
					 idProperty: "id",
					 root: "data",
        ...
        appMain.AssortmentStore.superclass.initComponent.apply(this, arguments);
    } // eo function initComponent
 
    ,onRender:function() {
        this.store.load();
 
        Application.AssortmentStore.superclass.onRender.apply(this, arguments);
    } // eo function onRender
});
 
//Ext.reg('AssortmentStore', appMain.AssortmentStore);

/*
From this point on we have a new component, a building block for our application that we can use everywhere (window, border panel region, standalone) where the list of personnel is needed.
 
We can create it either as:
	var dataStore = new appMain.AssortmentStore();
	 
	// or using it's xtype
	var grid = new Ext.Grid({
		 store:{xtype:'AssortmentStore'}
		,....
	});
*/
