...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<?php echo $form->labelEx($model,'StatusId'); ?> 
		<?php 	
			...
			elseif ($UserRole<=5 && $UserRole>3 && $CurrentStatusOrder<4)
			{	
	...
	<!--- КОНТРАГЕНТ --->
	<td>
		<span style="white-space:nowrap;"><label><?php echo $model->StatusId != Events::STATUS_COMPLETED ? Yii::t('general','User in system, whom to call') : Yii::t('general','User in system, who has been called');  ?></label></span>
		<?php 			
			if ($UserRole <= 2)  
			{
				...
				$this->widget('ext.select2.ESelect2', array(
						...
						'data' => CHtml::listData(User::model()->findAll( array(
							'order'=>'name ASC', 
							'condition' => $condition )), 'id','username'
							),
						'options'=> array('allowClear'=>true, 
							'width' => '200', 
							'placeholder' => '', 
			...
			elseif ($CurrentStatusOrder<4 && $UserRole<=5 && $UserRole>3)
			{				
